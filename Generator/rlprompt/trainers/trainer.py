...
class Trainer:
    def __init__(
        ...
        self.setting_theta = setting_theta
    ...
    def _train_step(
        ...
        loss, batch_log = model(
            batch, clean_prompt, target_label
        )
        loss.backward()
        self.train_op()
        ...
        for epoch in range(total_train_epochs):
            for step, batch in enumerate(train_dataloader):
                ...
                if self.do_eval and eval_by_steps and total_steps % self.df_steps == 0:
                    prompt_df_train = pd.DataFrame(columns=['prompt', 'sim', 'paa'])
                    prompt_df_val = pd.DataFrame(columns=['prompt', 'sim', 'paa'])
                    for key in prompt__dic_train.keys():
                        new_row = pd.DataFrame({
                            'prompt': [key[0]], 'sim': [prompt_dic_train[key][0]],
                            'paa': [prompt_dic_train[key][1]]
                        })
                        prompt_df_train = pd.concat([prompt_df_train, new_row], ignore_index=True)
                    for key in prompt_dic_val.keys():
                        new_row = pd.DataFrame({
                            'prompt': [key[0]], 'sim': [prompt_dic_val[key][0]],
                            'paa': [prompt_dic_val[key][1]]
                        })
                    ...
                    prompt_df_train.to_csv(os.path.join(self.save_dir, f'{total_steps}', 'prompt_dic_train.csv'), index=False)
                    prompt_df_val.to_csv(os.path.join(self.save_dir, f'{total_steps}', 'prompt_dic_val.csv'), index=False)
    ...
    def _get_eval_dataloader(self, eval_dataset: Dataset) -> DataLoader:
        return DataLoader(eval_dataset,
            ...
            prompt_dic_train: Dict[Tuple, Tuple] = {},
            prompt_dic_val: Dict[Tuple, Tuple] = {}
